...
\begin{abstract}
    ...
    The null-order comparison is made between architectures rather than between idealized responses. The transmission is derived from aperture positions and a beam-combiner matrix, so any physically realizable nulling design can be evaluated, and because the rotation average of a single-baseline geometry reduces to one dimension it is evaluated at the cost of the closed-form reference. Two consequences follow. A four-aperture array admits only one fourth-order output, and chopping requires two, so a fourth-order null is incompatible with LIFE's chopping scheme at four apertures; six are the minimum. Built from the same 4\,m collectors as the reference, and so carrying half again its collecting area, a six-aperture fourth-order design completes the programme about thirty percent sooner and tolerates a larger flat allowance at its own operating point; evaluated instead at equal total collecting area the same design is about sixteen percent slower, so the schedule gain belongs to the aperture count rather than to the null depth. What the null depth changes, under either convention, is where in the band the allowance should be spent. These values remain iso-mission-time amplitudes within tested budget families, not wavelength-by-wavelength subsystem requirements.
\end{abstract}
...
\begin{itemize}
    \item \textit{What is the dimension of the simulator's trade space, and can it be reduced without compromising fidelity?}
    \item \textit{Which source-model computations can be reused when only global filters or budget amplitudes change, and what does that reuse buy?}
    \item \textit{Can the response of an arbitrary, physically realizable nulling architecture be evaluated at the cost of the closed-form reference, rather than by a per-target spatial grid?}
    \item \textit{Can the maximum tolerated amplitude be located for flat and linear-ramp random-noise budgets at a fixed, deliberately pessimistic operating point?}
    \item \textit{Does the resulting requirement depend on the nulling architecture, and if so, in what way?}
\end{itemize}
...
Against that background, four things in this thesis are new. The first is the iso-mission-time inversion itself. The second is the closed-form treatment of the circularly symmetric backgrounds, which removes image size as a production-science parameter (Section \ref{sec: ts reduction: image spec}). The third is a transmission model that takes an interferometer's aperture positions and beam-combiner matrix rather than a fixed analytic response, so that any physically realizable nulling architecture can be evaluated, and evaluated at the cost of the closed-form reference rather than at the cost of a spatial grid (Section \ref{sec: methods: combiner}). The fourth is the finding that the spectral preference between budget shapes inverts with null order, survives the full scheduling and percentile chain rather than following only from the underlying leakage scaling, and survives the substitution of a realizable combiner for an idealized response.

The third of these deserves a remark, because it is what allows the central question to be answered as posed rather than in a weakened form. A requirement derived through a response written in closed form for one array is a requirement about that array. Only a model that accepts an arbitrary combiner can distinguish the part of the answer that belongs to the mission from the part that belongs to the instrument, and Section \ref{sec: tse: architecture} shows that the two differ substantially.

The remainder follows the causal chain from mission goal to engineering requirement. Section \ref{sec: background} establishes the experiment, target populations, measurement, SNR, and error budget. Section \ref{sec: methods} builds the simulator and the transmission model that lets it represent an arbitrary architecture. Section \ref{sec: ts reduction} reduces the raw trade space and derives the analytic background response. Section \ref{sec: tse} then presents the completed exploration, first for the reference double-Bracewell array and then for a realizable fourth-order design. The discussion and conclusion state what the resulting allowances do and do not imply for LIFE.
...
\chapteropening{Methods}{Building a simulator that is agnostic to both the noise mechanism and the array} \label{sec: methods}
...
\end{align}
with $N$ the number of apertures. Everything the simulator needs follows from Eq. \ref{eq: general transmission}: the null order, the chopping behaviour, the rotation-averaged background response, and the throughput. Nothing is assumed about the architecture beyond the array being lossless, which makes the rows of $U$ orthonormal.
...
Three designs are used in this work, summarized in Table \ref{tab: architectures}. The first is LIFE's reference array. The second and third achieve a fourth-order null, and comparing them establishes a constraint that no idealized response can express.
...
\bottomrule
\end{tabular}
\caption{Architectures evaluated in this work. Deep outputs and the fraction of collected light follow from the row structure of an orthonormal combiner matrix rather than from any fitted quantity; the $50\,\%$ and $25\,\%$ figures reproduce those quoted for Darwin and the Angel Cross by Guyon et al. \cite{guyon2013beam}.}
\label{tab: architectures}
\end{table}

The Angel Cross places two crossed Bracewell pairs on a square. Its four orthogonal outputs comprise one bright output, two second-order outputs, and a single output whose amplitude vanishes to second order and whose intensity therefore vanishes to fourth. That single deep output is the constraint: chopping requires two dark outputs whose difference changes sign for an off-axis source, and a four-aperture fourth-order array has only one. \textbf{A fourth-order null at four apertures is therefore incompatible with the chopping scheme LIFE relies on}, not as a matter of engineering difficulty but as a consequence of counting orthogonal outputs.

Six apertures are the minimum that restores it. The double triple nuller is the direct analogue of LIFE's own architecture with each two-aperture Bracewell pair replaced by a three-aperture nuller of amplitudes $1:-2:1$. A single such triple has amplitude proportional to $2(\cos\psi-1)$, second order in the offset, so its intensity vanishes to fourth order; this is the one-dimensional $2(N-1)$ rule for $N=3$ \cite{guyon2013beam}. Two triples separated along the imaging baseline and recombined at relative phase $\pi/2$, exactly as the two Bracewell pairs are in the reference design, yield two fourth-order outputs of opposite modulation sign, and chopping is available again.

Because the achievable null order is primarily a function of aperture number, a deeper null is bought with collectors rather than with a cleverer combiner at fixed $N$ \cite{guyon2013beam}. Kernel nullers formulated for an arbitrary number of apertures \cite{laugier2020kernel} are the natural family in which further designs would be expressed, and Eq. \ref{eq: general transmission} accepts them unchanged.
...
\bottomrule
\end{tabular}
\caption{Validation matrix for the AMS and analytic source calculation. Agreement with shared code establishes numerical consistency, not independent empirical validation of the astrophysical assumptions. The first, third and fourth rows were recomputed for this document from the committed source, and their scripts are listed in Appendix \ref{app: reproducibility}; the remaining entries are carried over from the development record and were not re-derived here.}
...
\caption{Construction of an imposed random-noise budget on top of the astrophysical background. The dashed green line is the imposed $N_I(\lambda)$, here a linear ramp that vanishes at the short-wavelength end of the band and reaches its full amplitude at the long-wavelength end; this is the long-weighted family of Section \ref{sec: tse: budget shape}. The solid green line is the resulting total background including the budget, and the shaded area between the two curves is the allowance itself. All three curves are photon-rate spectral densities referred to the pre-efficiency single-output plane, not variances. The configuration shown is Hab2Max at null order two and the common operating point, with the amplitude set to the largest value compatible with the 5.5-year mission-time target; the same panel recurs among the results as Fig. \ref{fig: tse budget shapes no2}(c). \emph{What the background curves belong to.} The imposed amplitude is a property of the whole ensemble, since the mission-time search that fixes it runs over the full catalog, but the background curves beneath it are not. The simulator's plot mode draws one target resolved by positional index (\texttt{ams.py:634-636}), which for both catalogs is the host star at 46.0\,pc with $T_\mathrm{eff}=6303$\,K, ecliptic latitude $-34.9^\circ$ and a habitable-zone centre of 0.977\,AU. That target is not typical of the eligible population: it lies at the 83rd percentile in distance and the 90th in effective temperature, so it is more distant and hotter than the median, and correspondingly less resolved. These figures should therefore be read as illustrating the construction, not as showing an average background. The ensemble-median background, which is the quantity to compare an allowance against, is reported in Section \ref{sec: discussion: budget scale} and tabulated in \texttt{thesis/reproducibility/background\_context.tsv}.}
...
Because local-zodiacal shot noise is present for every target, the change propagates nonlinearly through SNR, target ranking, and the 90th-percentile mission time. Comparing the corrected and uncorrected paths over every qualifying target, with the code otherwise identical, lowers the one-hour signal-to-noise ratio by a median of $9.2\%$ at null order two and $9.8\%$ at order four for \mbox{Hab2Max}, and by $9.1\%$ and $9.8\%$ for \mbox{Hab2Min}. The distribution is skewed rather than tight: the sixteenth-to-eighty-fourth percentile range runs from about $-11\%$ to $-2.7\%$, the small shifts belonging to targets whose noise budget the local-zodiacal term does not dominate. The scope of that figure should be read carefully: it applies to results computed through the default tapered local-zodiacal path, and the present work does not attempt to propagate it into any previously published LIFE yield number. Doing so would require rerunning those studies and is left to future work.
...
\end{figure} Thus spatial image size is no longer a science parameter, while wavelength-bin integration is explicitly converged for the retained binning. Appendix \ref{app: analytic derivation} records the derivation and numerical conventions in greater detail.

\subsection{Restriction of Architecture}
The transmission model of Section \ref{sec: methods: combiner} accepts any combiner, which makes the space of architectures as unbounded as the space of noise functions was before the first reduction. This study restricts it to two: the reference double Bracewell, and the six-aperture double triple nuller of Section \ref{sec: methods: architectures} as a realizable fourth-order design. The Angel Cross is excluded not by choice but by construction, since it offers a single deep output and therefore cannot chop.

The restriction is deliberate and it bounds the conclusions. Each architecture is evaluated at its own optimal baseline rather than at the reference's, so the comparison is between designs each doing the best it can, but two designs are not a survey.

The collector size, not the total area, is what is held fixed between them. Every design is built from the 4\,m collectors of Table \ref{tab: study assumptions}, so the six-aperture array has $75.4$\,m$^2$ of collecting area against the reference's $50.3$. This convention is forced by the simulator's own physics rather than chosen for convenience: the collecting area and the single-aperture field of view both follow from the collector diameter, the first as $N\pi(D/2)^2$ and the second as $\lambda/2D$, so fixing the total area instead would mean $3.27$\,m collectors whose beams subtend $1.5$ times the solid angle and therefore admit correspondingly more of the uniform local-zodiacal foreground. Holding the area fixed while keeping the reference's field of view would describe no instrument at all.

The consequence must be stated plainly, because it is not a like-for-like comparison: the six-aperture design is given half again as much collecting area, and part of what it gains is bought with that area rather than with the deeper null. The opposite convention is equally self-consistent and points the other way. Evaluated at equal total collecting area, with $3.27$\,m collectors, the same design reaches a Hab2Max zero-budget mission time of $6.18$\,yr against the reference's $5.32$, which is $16\,\%$ \textit{slower} rather than faster. The architecture comparison of Section \ref{sec: tse: architecture} is therefore bracketed by the two conventions, and which one is the right systems-level question is a mission-design matter this thesis does not settle. What survives both is the ordering of the budget families, which is the result the central question asks for.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_flat_5p5.pdf}
        \caption{Flat}
    \end{subfigure}
    \hfill
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_short_weighted_5p5.pdf}
        \caption{Short-weighted}
    \end{subfigure}
    ...
    \begin{subfigure}{.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_long_weighted_5p5.pdf}
        \caption{Long-weighted}
    \end{subfigure}
    \caption{Final budgets for the six-aperture fourth-order design, at the same Hab2Max operating point and target as Fig. \ref{fig: tse budget shapes no2} and at equal total collecting area. Geometric stellar leakage lies on the floor across the whole band, so the short-wavelength background no longer conceals added noise while the rising local-zodiacal term at long wavelengths still does; the long-weighted ramp consequently admits the largest endpoint. As in Figs. \ref{fig: budget example} and \ref{fig: tse budget shapes no2}, the background curves are those of the single plotted target rather than an ensemble average. It is the same target here as in Fig. \ref{fig: tse budget shapes no2}, so the architecture comparison between the two figures is unaffected by the choice; the same host star also happens to be the one plotted for Hab2Min.}
...
\toprule
Catalog & Architecture & Target & Flat & Short-weighted & Long-weighted \\
        &              & [yr]   & \multicolumn{3}{c}{Amplitude [ph\,s$^{-1}$\,$\upmu$m$^{-1}$]} \\
\midrule
Hab2Max & Double Bracewell & 5.5 & 142  & \textbf{300}  & 211  \\
Hab2Max & Double Bracewell & 6.0 & 584  & \textbf{1300} & 1090 \\
Hab2Max & Triple nuller    & 4.0 & 241  & 403  & \textbf{557}  \\
Hab2Max & Triple nuller    & 4.5 & 702  & 1380 & \textbf{1760} \\
\addlinespace
Hab2Min & Double Bracewell & 7.5 & 60.6 & \textbf{198}  & 157  \\
Hab2Min & Double Bracewell & 8.0 & 461  & \textbf{855}  & 812  \\
Hab2Min & Triple nuller    & 5.5 & 177  & 226  & \textbf{602}  \\
Hab2Min & Triple nuller    & 6.0 & 409  & 734  & \textbf{1310} \\
\bottomrule
\end{tabular}
\caption{Final amplitudes for the second-order reference array and for the six-aperture fourth-order design of Section \ref{sec: methods: architectures}, each sized to its own response peak and each evaluated at \emph{its own} mission-time targets. Both designs are built from 4\,m collectors, so the six-aperture array carries $75.4$ against $50.3$\,m$^2$; Section \ref{sec: methods: architectures} states that convention and its consequences. Targets follow the rule used throughout: the lowest half year above that configuration's zero-budget time, which is $5.32$ and $7.41$\,yr for the reference and $3.72$ and $5.09$\,yr for the deeper null. Comparing the two at a common target would place the fourth-order design far from its own feasibility boundary, where the search saturates and the endpoint ceases to be informative. The largest endpoint in each row is set in bold: the short-weighted family for the reference, the long-weighted family for the deeper null, in all eight rows. A ramp value is its non-zero imposed endpoint. Values are quoted to three significant figures, and both halves come from the same general-combiner path so that the comparison shares a provenance. These endpoint parameters compare sensitivity within the tested families; they are not common integrated-noise measures or engineering-cost rankings.}
...
Relaxing the target by half a year increases the allowance sharply for either architecture, but not by the same factor. Each configuration is compared against its own primary target, so both start the same distance above their own feasibility boundary. The Hab2Max flat budget grows from 142 to 584 for the reference, a factor of $4.1$, and from 241 to 702 for the triple nuller, a factor of $2.9$; for Hab2Min it rises from 60.6 to 461, a factor of $7.6$, against 177 to 409, a factor of $2.3$. The reference therefore converts additional schedule into tolerated noise appreciably more readily, which is the counterpart of the result in Section \ref{sec: tse: architecture}: the deeper null buys its margin in schedule rather than in noise tolerance, so relaxing the schedule returns less to it than to the shallower design.

A ramp endpoint and a flat amplitude are not directly comparable quantities, so it is worth checking that the reversal is not an artifact of the chosen reporting convention. A ramp rising linearly from zero to $A$ across the band has mean $A/2$, so a flat budget of $A/2$ adds the same band-integrated noise; the integral-neutral ramp endpoint is therefore twice the flat amplitude in the same row. Measured against that reference, the long-weighted endpoint exceeds the neutral value at null order four in all four rows while the short-weighted endpoint never does, so at order four the reversal holds under equal integrated added noise and not only under equal endpoints.

At order two the same test is weaker, and the way it fails is itself informative. The short-weighted endpoint clears the neutral value for Hab2Max at both targets and for Hab2Min at 7.5\,yr, but not for Hab2Min at 8.0\,yr, where 855 falls short of the neutral 923. That single cell is also where the two independent runs of the reference array disagree most: the production campaign returned 970 for it against this run's 855, a 12\,\% difference between two calculations that agree to $1.2\times10^{-4}$ in mission time. The verdict for that cell therefore flips depending on which run is quoted, which is the amplification of Appendix \ref{app: reproducibility} appearing in a place where it changes a stated conclusion rather than only a number. The honest statement is that the order-two preference for short wavelengths is clear in endpoint terms in all four rows, and clear in integral-neutral terms in three of them.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_linear_5p5.pdf}
        \caption{Order 4, Hab2Max}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_lo_linear_7p5.pdf}
...
\subsection{Dependence on the Agnostic Parameters} \label{sec: tse: agnostic}
...
The fourth-order grids quantify that asymmetry directly. At the adopted field of regard of $65^\circ$, sweeping the limiting magnitude across the full range from 6 to 9 changes the allowance by a factor of only 1.21 for Hab2Max and 1.86 for Hab2Min, and the Hab2Max value is constant at 135\,ph\,s$^{-1}\,\upmu$m$^{-1}$ for every magnitude above 6.4. Sweeping the field of regard at fixed magnitude 7 changes it by 2.44 and 2.80 over the same grids. For this architecture the requirement is therefore set by pointing access far more than by target brightness, and its iso-budget contours are correspondingly close to vertical.

The two architectures share that qualitative dependence but not its extent, and the difference is smaller than the panels' differing colour scales suggest. The fourth-order design is feasible one grid step further into restrictive pointing, down to $61^\circ$ against the reference's $65^\circ$; since the grid spacing is $3.6^\circ$, that margin is resolved but not measured, and only a finer scan would place the edge more precisely. Where pointing is generous the ordering inverts: the fourth-order allowance peaks at 331 and 210\,ph\,s$^{-1}\,\upmu$m$^{-1}$ for the two catalogs, roughly 0.6 and 0.46 of the reference peaks read from the corresponding colour bars. This is the same trade seen at the operating points of Table \ref{tab: budget shapes}, now resolved over the plane: the deeper null buys access under tight constraints and gives back allowance under loose ones. Notably, the lowest feasible field of regard for the fourth-order design does not move with limiting magnitude at all, remaining at $61^\circ$ across the entire magnitude range in both catalogs.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_mag_5p5.pdf}
        \caption{Order 4, Hab2Max, 5.5\,yr}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_lo_mag_7p5.pdf}
        \caption{Order 4, Hab2Min, 7.5\,yr}
    \end{subfigure}
    \caption{Final iso-mission-time flat-budget scans over limiting magnitude and field of regard, with slew time fixed at 12\,h. Panels (a) and (b) are the second-order reference array; panels (c) and (d) are the six-aperture fourth-order design of Section \ref{sec: methods: architectures}, each sized to its own response peak and at equal total collecting area. Each grid is $15\times15$ points, so the field-of-regard spacing is $3.6^\circ$. Dark regions have no admissible positive budget at the corresponding operating point; the fourth-order design reaches one grid step further into restrictive pointing, at $61^\circ$ against $65^\circ$, in both catalogs. \textbf{Colour scales differ between panels} and peak at 331 and 210\,ph\,s$^{-1}\,\upmu$m$^{-1}$ for the fourth-order panels against roughly 560 and 460 for the reference, so the fourth-order maps are darker at equal allowance. The recovered grids are tabulated in \texttt{thesis/reproducibility/stageb\_grids.tsv}.}
...
Slew time is by a wide margin the more damaging of the two agnostic parameters examined in this section. At the adopted field of regard of $65^\circ$, the fourth-order allowance falls by a factor of 14.4 for Hab2Max and 24.0 for Hab2Min across the tested slew range, against the factors of 1.21 and 1.86 that the entire magnitude range produces on the same architecture. Overhead, unlike target brightness, is charged on every visit and cannot be recovered by the scheduler.

Both architectures produce the same wedge: feasibility requires either a short slew or a wide field of regard, and fails in the upper-left corner where neither holds. The fourth-order design occupies a slightly larger wedge, staying feasible out to 20\,h of slew where the reference is exhausted near 17\,h, and reaching down to $47^\circ$ and $51^\circ$ of pointing access at the shortest slews. Only 41.8\,\% and 39.1\,\% of the two grids admit any positive budget at all, so most of this plane is not a trade at any noise level but simply infeasible. Inside the wedge the ordering again reverses, the fourth-order allowance peaking at 983 and 741\,ph\,s$^{-1}\,\upmu$m$^{-1}$ against the reference's larger values. The pattern is consistent with Section \ref{sec: tse: architecture}: the deeper null helps where the schedule is the binding constraint and not where it is comfortable.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_hi_slew_5p5.pdf}
        \caption{Order 4, Hab2Max, 5.5\,yr}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/tse/physical/phys_lo_slew_7p5.pdf}
        \caption{Order 4, Hab2Min, 7.5\,yr}
    \end{subfigure}
    \caption{Final iso-mission-time flat-budget scans over slew time and field of regard at limiting magnitude 7. Panels (a) and (b) are the second-order reference array, panels (c) and (d) the six-aperture fourth-order design, at equal total collecting area and each at its own baseline prescription. Dark regions have no admissible positive budget: 58.2\,\% and 60.9\,\% of the two fourth-order grids are infeasible at any noise level. Both architectures give the same wedge, feasible only where the slew is short or the field of regard wide; the fourth-order design holds out to 20\,h of slew against the reference's 17\,h, while peaking lower inside the wedge. \textbf{Colour scales differ between panels.} Recovered grids in \texttt{thesis/reproducibility/stageb\_grids.tsv}.}
...
\begin{figure}[htbp]
    ...
    \caption{Ecliptic view of the final Hab2Max selection at limiting magnitude 7 and FoR $65^\circ$, for the second-order reference array. Here ``interesting'' denotes Experiment-1-eligible targets: 3605 host stars. \emph{Reading the axes.} The radial coordinate is the distance to the host star in parsec, with gridlines every 10\,pc; the polar angle is ecliptic \emph{latitude}, and stars at ecliptic longitude beyond $180^\circ$ are reflected through the origin so that both longitude hemispheres share the plane. Ecliptic longitude is therefore not resolved and the figure is not a sky map. The dashed orange line is the ecliptic plane and the shaded wedges are the zone of avoidance, $|\text{latitude}|>65^\circ$, which is how the field of regard enters the model (\texttt{ams.py:393}). \emph{Reading the percentage.} The annotation counts eligible host stars given zero SNR by \emph{either} exclusion, since one mask applies both. Decomposed over the same 3605 stars: the field of regard alone accounts for 6.3\,\%, the magnitude limit alone for 19.2\,\%, and both together for 1.3\,\%, giving marginal totals of 7.5\,\% and 20.5\,\%. \textbf{The loss is therefore dominated by the magnitude limit, not by the zone of avoidance the figure draws.} Reproduced by \texttt{lifesim/analysis/test\_scripts/decompose\_target\_loss.py}; the value is specific to this catalog, these cuts and this experiment definition.}
    \label{fig: tse yield}
\end{figure}
...
Table \ref{tab: null order synthesis} collects the effects that change between the two architectures alongside their consequences for the reported allowance. A deeper null does not act on the stellar term alone. The far-field average of the response falls from $1/4$ to $1/6$, which is one over the aperture count in each case and the general result for a lossless combiner, so every background term is transmitted less; the uniform local-zodiacal contribution is measured to scale between the two by $0.750$, exactly the ratio of the far-field averages of the underlying envelopes. Superposed on that common reduction, and only for the resolved stellar disk, the fourth-order dependence removes the leakage almost entirely.
...
Light in the chopping pair & 50\,\% (two of four outputs) & 33\,\% (two of six outputs) \\
...
Zero-budget mission time, Hab2Max & 5.32\,yr & 4.98\,yr \\
\bottomrule
\end{tabular}
\caption{Synthesis of the physical changes between the reference array and the six-aperture fourth-order design, and their consequences for the reported allowance.}
...
\midrule
Hab2Max & Double Bracewell     & 50.3 & 5.32 & 5.5 & 142 & 211 \\
Hab2Max & Double triple nuller & 75.4 & \textbf{3.72} & 4.0 & \textbf{241} & \textbf{557} \\
Hab2Min & Double Bracewell     & 50.3 & 7.41 & 7.5 & 60.6 & 157 \\
Hab2Min & Double triple nuller & 75.4 & \textbf{5.09} & 5.5 & \textbf{177} & \textbf{602} \\
\bottomrule
\end{tabular}
\caption{Reference and fourth-order architectures, each sized to its own response peak and each at its own primary mission-time target, the lowest half year above its zero-budget time. Amplitudes are in ph\,s$^{-1}$\,$\upmu$m$^{-1}$. \textbf{The two designs share a collector diameter of 4\,m, not a total collecting area}, so the six-aperture array collects half again as much light; Section \ref{sec: methods: architectures} explains why that convention is forced and what the opposite one gives. Because the targets differ, the amplitude columns are not a like-for-like comparison either: they state what each design tolerates where it actually operates.}
...
Half again the collecting area would by itself raise every target's signal-to-noise ratio by roughly $\sqrt{1.5}=1.22$ in a background-limited regime, so the median $1.31$ is close to what the extra aperture alone would deliver, and the excess at the top of the distribution is what the deeper null contributes. That excess is where the physics is: the best targets are the nearest and most resolved stars, so they are the ones whose noise is dominated by geometric stellar leakage, and they are the ones a fourth-order null helps most. The schedule gain therefore has two separable causes, and only the smaller of them is attributable to null depth.
...
Two boundaries limit the comparison. The equal-area convention means the six-aperture design is six smaller collectors rather than six of the reference size, and whether that is the appropriate systems-level trade is not settled here. And two architectures are a comparison, not a survey: Eq. \ref{eq: general transmission} admits an arbitrary combiner, and the space of designs that satisfy the chopping requirement has not been explored.
...
The consequence for the requirement is that the two architectures are not as different as the raw allowances imply. The reference's flat allowance is roughly $3.3\,\%$ of its background for Hab2Max and $1.4\,\%$ for Hab2Min. The six-aperture design's are about $7.7\,\%$ and $5.7\,\%$ of its own, larger but of the same order. In both cases the instrument must be quiet to within a few percent of an irreducible foreground, which is a demanding requirement for either design. A deeper null relaxes it by a factor of two or three, not by the order of magnitude that a comparison against an idealized response would suggest.
...
That last exposure carries a caution that outlives this study. An iso-mission-time requirement is defined where the completion curve is steep, so it inherits an amplified version of any error in the background it is measured against: a $27.3\%$ error in one term moved the derived allowance by up to a factor of eleven (Table \ref{tab: localzodi impact}), and the amplification is largest exactly where the target is most demanding and the result therefore most interesting. Requirements derived this way should be quoted with the background model that produced them, and a background correction should be treated as invalidating them rather than perturbing them.
...
\midrule
1 & Two architectures, not a survey, and one sizing convention & The requirement is shown to depend on the array, but only two designs are evaluated. Collector diameter rather than total area is held fixed, so aperture count and collecting area are confounded: the six-aperture design has half again the reference's area, and the schedule advantage reverses in sign under the opposite convention (Section \ref{sec: methods: architectures}). Separating null depth from aperture count needs a survey over both, which this work does not attempt. \\
...
\subsection{Outlook}
The next scientific step is a wider survey of architectures rather than another run of the present trade space. Two designs establish that the requirement depends on the array, but not how it depends on it: the transmission model accepts any combiner, and the space of designs that satisfy the chopping requirement is unexplored. Kernel nullers formulated for arbitrary aperture number \cite{laugier2020kernel} are the natural family in which to conduct that survey, and the equal-area convention adopted here should be relaxed so that aperture count and collector size trade against each other explicitly. Optical losses and instability terms, which no design in this work carries, belong in the same extension. The aggregate allowance should then be allocated to concrete subsystems at the same reference plane.
...
The completed AMS and TSE answer the central research question for every tested budget family and operating point. Under the analytic background model of Section \ref{sec: ts reduction: image spec}, the nominal second-order null permits primary flat allowances of 142\,ph\,s$^{-1}$\,$\upmu$m$^{-1}$ for Hab2Max at 5.5\,yr and 60.6 for Hab2Min at 7.5\,yr. A realizable six-aperture fourth-order design, built from the same 4\,m collectors and therefore carrying half again the collecting area, permits 241 and 177 at its own targets of 4.0 and 5.5\,yr, reaching those targets some thirty percent sooner than the reference reaches its own; and the least damaging tested ramp reverses from short-weighted to long-weighted.
...
That last quantity matters for interpretation. The search stops once the evaluated mission time lies within 0.05\,yr of the target, and it returns its last accepted amplitude, so a reported value belongs to the achieved time rather than to the nominal target. Across the twenty-four reruns the residuals span $-0.045$ to $+0.045$\,yr, filling the stopping tolerance almost exactly. Converting a residual into an amplitude uncertainty requires the local slope of amplitude against mission time, which the secant between each catalog's two targets estimates as roughly $6\times10^{2}$ to $1.5\times10^{3}$\,ph\,s$^{-1}$\,$\upmu$m$^{-1}$ per year. The stopping tolerance therefore corresponds to an amplitude uncertainty of order 20 to 80\,ph\,s$^{-1}$\,$\upmu$m$^{-1}$, depending on the row. Because the response is not linear in mission time, these figures indicate the scale of the search uncertainty rather than a calibrated interval, and they are the reason no reported amplitude should be read to better than its leading two digits.
...
Hab2Max & 5.5 & 138.1 & 181.4 & 347.1 & 5.511--5.554 \\
Hab2Max & 6.0 & 394.5 & 698.1 & 890.5 & 5.994--6.037 \\
Hab2Min & 7.5 & 129.3 & 210.3 & 307.8 & 7.488--7.523 \\
Hab2Min & 8.0 & 221.9 & 381.1 & 641.2 & 7.925--8.028 \\
\bottomrule
\end{tabular}
