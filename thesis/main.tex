...
It is worth being explicit that this charge does not double-count the proxy's own far-field reduction. Transmission and throughput are separate quantities and enter the calculation as independent factors: the $\sin^n$ response is a dimensionless function of angle describing where light is directed, chosen to impose a fourth-order null near the line of sight rather than derived from a physical combiner, whereas the efficiency collects the fraction of gathered photons that reaches a detector at all. The change from $1/4$ to $3/16$ in the far-field average belongs to the first and is already present in the unpenalized order-four results; the factor applied here belongs to the second and is absent from them.
...
Three boundaries limit how far this conclusion can be carried. The baseline prescription constant of Section \ref{sec: methods: AMS} is derived for a second-order double Bracewell and is retained unchanged at order four, so neither arm is baseline-optimal for the deeper null and the measured penalty is an upper bound. More importantly, the efficiency factor is specific to a fixed aperture count and is not a general law relating null order to throughput. The $25\%$ figure belongs to the four-aperture Angel Cross, where only one of four outputs carries the deep null. Guyon et al.\ show that the achievable null order is primarily a function of the number of apertures, and that a six-aperture combiner can be designed to produce three fourth-order outputs out of six, recovering $50\%$ of the planet light and therefore matching the second-order reference used here \cite{guyon2013beam}. The penalty applied in this section is consequently the cost of deepening the null \emph{at fixed aperture count}, which is the pessimistic end of the available range rather than an unavoidable charge. A design that escapes it does so by adding collectors, not by improving the combiner at four. The comparison becomes more useful when inverted. Instead of asking what a particular efficiency does to the mission, one can ask what efficiency a fourth-order architecture must retain for its deeper null to shorten the mission at all. Solving $t_4(\tau)=t_2(\tau_\mathrm{nom})$ at zero added budget, where $\tau_\mathrm{nom}=0.15$ is the reference optical throughput, gives $\tau^\star=0.118$ for Hab2Max and $0.117$ for Hab2Min. The deeper null therefore pays for itself only if the architecture delivering it retains about $79\%$ and $78\%$ respectively of the second-order throughput. Nothing in the calculation forces the two catalogs to agree, and their agreement to within one percentage point indicates that the criterion belongs to the instrument model rather than to the target sample.

Stated this way the result is a test that any proposed architecture can be put to, rather than a statement about one penalty factor. The four-aperture Angel Cross retains $25\%$ of the collected light against the $50\%$ of the second-order reference, a retention of one half, and so fails the criterion by a wide margin; that is why the penalized configuration above cannot complete the programme at any target. A six-aperture combiner of the kind described by Guyon et al., which places the deep null on three of its six outputs, retains the same $50\%$ fraction as the second-order reference and therefore satisfies the criterion at equal collecting area \cite{guyon2013beam}. The conclusion is consequently not that fourth-order nulling is unattractive, but that it is unattractive at four apertures. Kernel nullers formulated for an arbitrary number of apertures \cite{laugier2020kernel} are the natural class in which the criterion could be evaluated for a concrete, realizable combiner rather than for the shape proxy used here.

The third question, whether the spectral reversal of Section \ref{sec: tse: null order} is itself an artifact of the unnormalized throughput, can be answered rather than left open. The reversal concerns the ordering among budget families, not their absolute size, so it remains testable at targets the penalized configuration can actually reach. Repeating the search above each catalog's penalized zero-budget time gives Table \ref{tab: penalized reversal}.
...
\begin{tabular}{@{}llrrrr@{}}
\toprule
Catalog & Target [yr] & Flat & Short-weighted & Long-weighted & Ratio \\
\midrule
Hab2Max & 8.0  & 38  & 55  & 143 & 2.6 \\
Hab2Max & 8.5  & 134 & 249 & 411 & 1.7 \\
Hab2Max & 9.0  & 278 & 641 & 838 & 1.3 \\
Hab2Min & 11.0 & 49  & 81  & 154 & 1.9 \\
Hab2Min & 11.5 & 109 & 154 & 378 & 2.5 \\
Hab2Min & 12.0 & 185 & 277 & 626 & 2.3 \\
\bottomrule
\end{tabular}
\caption{Budget amplitudes in the throughput-penalized order-four configuration, at mission-time targets above its zero-budget time. Amplitudes are in ph\,s$^{-1}$\,$\upmu$m$^{-1}$; the ratio is long-weighted to short-weighted. The ordering established at full efficiency is preserved throughout.}
\label{tab: penalized reversal}
\end{table}

The long-weighted endpoint is the largest in all six cases, by factors of 1.3 to 2.6, and exceeds the integral-neutral reference of twice the flat amplitude in every one; the short-weighted endpoint clears that reference only in the most relaxed Hab2Max case. The spectral preference is therefore a property of the deeper null itself and not of the throughput the proxy leaves unnormalized. What the penalty removes is the absolute gain, not the shape of the requirement: a fourth-order design cannot reach the mission-time targets of this study, but where it can complete the programme at all, it still tolerates added noise most readily at long wavelengths.
...
\begin{itemize}
    ...
    \item That improvement does not survive the throughput a deeper null costs. Charging the order-four configuration the efficiency penalty of a realizable four-aperture combiner raises the mission time required with no added noise beyond every target studied here, so no allowance can be reported at all, and the penalty exceeds the null-depth benefit by three to five times (Section \ref{sec: tse: throughput}). The reported order-four allowances are therefore an upper bound on what a deeper null can buy, and at four apertures the engineering conclusion is the opposite of the one the raw comparison suggests. The penalty is a cost of deepening the null at fixed aperture count rather than an intrinsic one, since a combiner with more collectors can reach the same null order without it. Expressed as a requirement, a fourth-order architecture must retain roughly $78\%$ of the second-order throughput before its deeper null shortens the mission at all; the four-aperture arrangement retains half of it and a six-aperture one retains all of it. The spectral ordering itself survives the penalty wherever the programme can be completed at all.
\end{itemize}
