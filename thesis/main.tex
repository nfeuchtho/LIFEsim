...
They also make the inversion possible at all, and the margin is not marginal. Measured against the pre-rewrite source model on the same catalog subset, the same instrument settings and a single core, replacing the spatial grid by the closed-form background reduces the cost of a signal-to-noise evaluation from $93$ to $3.7$\,ms per planet, a factor of $25$. Extrapolated to the full catalog that is a fall from about nineteen hours to three quarters of an hour. Layering the vectorized, host-memoized evaluation of the simulator on top brings a complete catalog pass to roughly fifteen seconds, some three orders of magnitude below the analytic instrument alone and more than three thousand times faster than the grid it replaced. The comparison is made at an image size of 256; the grid cost scales as its square, so a larger grid widens the gap further.
