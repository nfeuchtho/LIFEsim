...
\toprule
Catalog & Target & Architecture & Flat & Short-weighted & Long-weighted \\
        & [yr]   &              & \multicolumn{3}{c}{Amplitude [ph\,s$^{-1}$\,$\upmu$m$^{-1}$]} \\
\midrule
Hab2Max & 5.5 & Double Bracewell     & 140 & \textbf{300}  & 210  \\
Hab2Max & 5.5 & Triple nuller        & 140 & 180  & \textbf{350}  \\
Hab2Max & 6.0 & Double Bracewell     & 610 & \textbf{1300} & 1090 \\
Hab2Max & 6.0 & Triple nuller        & 390 & 700  & \textbf{890}  \\
Hab2Min & 7.5 & Double Bracewell     & 65  & \textbf{200}  & 150  \\
Hab2Min & 7.5 & Triple nuller        & 130 & 210  & \textbf{310}  \\
Hab2Min & 8.0 & Double Bracewell     & 460 & \textbf{970}  & 810  \\
Hab2Min & 8.0 & Triple nuller        & 220 & 380  & \textbf{640}  \\
\bottomrule
\end{tabular}
\caption{Final amplitudes at the common operating point, for the second-order reference array and for the six-aperture fourth-order design of Section \ref{sec: methods: architectures}, each sized to its own response peak and at equal total collecting area. The largest endpoint in each row is set in bold: the short-weighted family for the reference, the long-weighted family for the deeper null, without exception. A ramp value is its non-zero imposed endpoint. These endpoint parameters compare sensitivity within the tested families; they are not common integrated-noise measures or engineering-cost rankings. Reference values are those of the production campaign; fourth-order values are from the architecture runs recorded in Appendix \ref{app: reproducibility}.}
...
Relaxing the target time increases the allowance sharply for either architecture, but not by the same factor. The Hab2Max flat budget grows from 140 to 610 for the reference and from 140 to 390 for the triple nuller when the target moves from 5.5 to 6\,yr; for Hab2Min it rises from 65 to 460 and from 130 to 220 between 7.5 and 8\,yr. The reference therefore converts additional schedule into tolerated noise more readily, which is the counterpart of the result in Section \ref{sec: tse: architecture}: the deeper null wins where the schedule is tight and the best targets set the pace, and loses where the schedule is loose and the marginal targets do.
...
Table \ref{tab: architecture comparison} reports the outcome at the common operating point. The fourth-order design completes Experiment~1 in less time than the reference with no added budget, by $6.4\,\%$ for Hab2Max and $6.0\,\%$ for Hab2Min, while tolerating a comparable flat allowance at the primary target.
...
\midrule
1 & Two architectures, not a survey & The requirement is shown to depend on the array, but only two designs are evaluated, and total collecting area is held equal so that aperture count and collector size do not trade against each other. \\
