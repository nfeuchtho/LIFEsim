...
# Slower, but still optimized version of the AHGS optimizer. About 4x faster.
...
class AhgsModule(SlopeModule):
    ...
    def observe_star(self,
        ...
        if not delete:
            ...
            if nstar not in self.star_data:
                data = dict()
                data['t_slew'] = self.data.catalog.loc[mask, 't_slew'].iloc[0]
                data['int_time'] = self.data.catalog.loc[mask, 'int_time'].iloc[0]
                self.star_data[nstar] = data
            else:
                data = self.star_data[nstar]
            ...
            # Check if modified planets have been detected for the first time
            detect_mask = ((self.data.catalog.detected == False)
                           & (self.data.catalog.snr_current >= self.data.options.optimization['snr_target']))

            if self.data.catalog.loc[detect_mask].shape[0] > 0:

                self.data.catalog.loc[detect_mask, 'detected'] = True
                self.data.catalog.loc[detect_mask, 't_detected'] = self.tot_time + int_time
                self.data.catalog.loc[detect_mask, 't_slew'] = self.star_data[nstar]['t_slew']
                self.data.catalog.loc[detect_mask, 'int_time'] = self.star_data[nstar]['int_time']
                ...
                for _, i in enumerate(np.where(detect_mask)[0]):
